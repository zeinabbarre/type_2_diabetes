    ...
    if population.lower() == "south asian" and snps:
        ...
        def get_tajima_for_position(chr_pos, table_tajima_values):
            ...
            try:
                ...
                print(f"❌ Error: chr_pos '{chr_pos}' is not a valid integer!")
                return 'N/A'
            for bin_start, tajima_d in table_tajima_values.items():
                try:
                    bin_start =int(bin_start)
                except ValueError:
                    print(f"❌ Error: bin_start '{bin_start}' is not a valid float!")
                    continue
...
def get_tajimas(chromosome, start_pos, end_pos, table_name):
    ...
    try:
        # ✅ Ensure chromosome is treated as a string when querying (important for X chromosome)
        df = pd.read_sql_query(query, conn, params=(str(chromosome), start_pos, end_pos))
        
        # ✅ Convert bin_start to integer (to prevent str-int comparison errors)
        df["bin_start"] = df["bin_start"].astype(int)

        print(df.head())  # 🔥 Debugging: Check what is inside the DataFrame
